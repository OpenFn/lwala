...
upsert("Person__c","CommCare_ID__c", fields(
  ...
  field("Education_Level__c", function(state){
    return return(dataValue("$.form.Basic_Information.Education_Level")(state).toString().replace(/_/g," "));
  }),
  ...
  field("Chronic_illness__c", function(state){
    return dataValue("$.form.Basic_Information.person_info.chronic_illness")(state).toString().replace(/ /g,";");
  }),
  ...
  field("Other_disability__c",function(state){
    var other = dataValue("Basic_Information.person_info.other_disability")(state);
    const toTitleCase = other.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    return toTitleCase;
  }),
  ...
  //field("Delivery_Facility__c",dataValue("form.TT5.Child_Information.Delivery_Information.Birth_Facility")),
